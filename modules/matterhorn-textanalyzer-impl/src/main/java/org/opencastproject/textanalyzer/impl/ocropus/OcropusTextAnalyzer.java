...
public class OcropusTextAnalyzer {
   ...
   */
  public OcropusTextFrame analyze(File image) throws TextAnalyzerException {
    if (binary == null)
    ...
    // Analyze
    ProcessExecutor<TextAnalyzerException> analyzer = new ProcessExecutor<TextAnalyzerException>(binary, getAnalysisOptions(image)) {
      @Override
    ...
    try {
      ...
      throw new TextAnalyzerException("Error running text analyzer " + binary, e);
    }
   ...
   */
  protected void postProcess() throws TextAnalyzerException {
    try {
