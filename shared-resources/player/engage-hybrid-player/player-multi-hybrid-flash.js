...
Opencast.FlashVersion = (function ()
{
    ...
    function AC_AddExtension(src, ext)
    {
        ...
        if (qIndex != -1)
        {
            ...
            if (path.length >= ext.length && path.lastIndexOf(ext) == (path.length - ext.length)) return src;
            else
        ...
        else
        {
            ...
            if (src.length >= ext.length && src.lastIndexOf(ext) == (src.length - ext.length)) return src; // Already have extension
            else
            return src + ext;
        }
    }
    ...
    { // flash is too old or we can't detect the plugin
        var alternateContent = 'Matterhorn requires Adobe Flash Player.<br>' + 'Please install latest version for your OS.<br>(recommended version 10.1)' + '<a href="http://www.adobe.com/go/getflash/">Get Flash</a>';
