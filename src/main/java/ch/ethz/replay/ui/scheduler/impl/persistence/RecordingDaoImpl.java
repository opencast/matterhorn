 ...
 * 
 */
public class RecordingDaoImpl extends TransactionalGenericHibernateDao<Recording, Long> implements RecordingDao {
  ...
  public long countByUpdatedAfter(final String locationName, final Date date) {
    ...
    Integer result = (Integer) getHibernateTemplate().execute(new HibernateCallback() {
      ...
      public Object doInHibernate(Session session) throws HibernateException, SQLException {
        Criteria c = session.createCriteria(RecordingImpl.class).setProjection(Projections.rowCount());
    ...
    if (result == 0 && locationName != null) {
      ...
      result = ((Long) getHibernateTemplate().execute(new HibernateCallback() {
        ...
        public Object doInHibernate(Session session) throws HibernateException, SQLException {
          return session.createQuery("select count(*) from Deletion where lastUpdated > :date").setDate("date", date)
                  .uniqueResult();
        }
      })).intValue();
    }
        ...
        return session
                .createSQLQuery(
                        "select max(u) from ( "
                                ...
                                + "UNION "
                                + "select max(d.lastupdated) as u from deletion d where d.locationid = :lid " + ") s")
                .setLong("lid", locationId).uniqueResult();
      }
    });
    return lastUpdated;
  }
   ...
   * <p/>
   * {@link RecordingFilter#getFreeText()} is searched in the summary of document and Dublin Core attachments, the
   * family name and given name of contact persons and the recording's title.
   */
  @Override
  @Transactional(readOnly = true)
  public List<Recording> findBy(Object filter) {
    if (filter instanceof RecordingFilter) {
      ...
      return Tool.cast(getHibernateTemplate().execute(new HibernateCallback() {
        ...
        public Object doInHibernate(Session session) throws HibernateException, SQLException {
          Criteria c = session.createCriteria(getEntityClass()).addOrder(Order.asc("startDate"))
          ...
          if (f.getSeriesId() != null)
            c.createAlias("series", "s", CriteriaSpecification.LEFT_JOIN).add(Restrictions.eq("s.id", f.getSeriesId()));
          if (f.getJobId() != null)
          ...
          if (f.getFreeText() != null) {
            ...
            c.createAlias("dublinCoreAttachment", "d", CriteriaSpecification.LEFT_JOIN);
            c.createAlias("contactPersons", "p", CriteriaSpecification.LEFT_JOIN);
            c.add(Restrictions.disjunction().add(Expression.ilike("title", f.getFreeText(), MatchMode.ANYWHERE))
  ...
  public Recording getByJobId(final String jobId) {
    return (Recording) getHibernateTemplate().execute(new HibernateCallback() {
      ...
      public Object doInHibernate(Session session) throws HibernateException, SQLException {
        return session.createQuery("from Recording where jobId = :j").setString("j", jobId).uniqueResult();
      }
    });
  }
  ...
  public List<Location> getAllScheduledLocations() {
    return getHibernateTemplate().executeFind(new HibernateCallback() {
      ...
      public Object doInHibernate(Session session) throws HibernateException, SQLException {
        return session.createQuery("select distinct r.location from Recording r " + "order by r.location.name asc")
                .list();
      }
    });
  }
  ...
  public List<Location> getAllScheduledLocationsLike(String like) {
    ...
    return getHibernateTemplate().executeFind(new HibernateCallback() {
      ...
      public Object doInHibernate(Session session) throws HibernateException, SQLException {
        return session
                .createQuery(
                        "select distinct r.location from Recording r " + "where lower(r.location.name) like :location "
                                + "order by r.location.name asc").setString("location", likeLower).list();
      }
    });
  }
}
