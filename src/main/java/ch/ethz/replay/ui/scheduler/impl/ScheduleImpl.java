...
/**
 * Todo {@link #schedule(ch.ethz.replay.ui.scheduler.Recording)} uses merge() to implement conversation
 * needed by components like the VVZ connection. Check, if this is a suitable strategy, or if it would
 * be better to extend the persistence context by implementing the session-per-conversion pattern, like
...
public class ScheduleImpl implements Schedule {
    ...
    private void saveRecording(Recording recording) {
        ...
        if (dbLocation.size() == 1) recording.setLocation(dbLocation.get(0));
    ...
    /**
     * This implementation tries at first to retrieve a location with the given name from the database.
     * If none can be found it creates a new one.
     */
