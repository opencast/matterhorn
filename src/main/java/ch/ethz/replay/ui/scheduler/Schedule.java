...
/**
 * The Schedule manages all scheduled recordings and is the main service for clients to deal with.
 * Therefore it should be implemented as a singleton.
 *
...
public interface Schedule {
    ...
    /**
     * Puts a recording on the schedule.
     * Also use this method to save updates to already scheduled recordings.
    ...
    /**
     * Put a whole series of recordings on the schedule.
     * Also use this method to save updates to already scheduled recordings.
     * todo Exception
     *
    ...
    /**
     * Removes the recording from the schedule.
     * todo Exception
     *
    ...
    /**
     * Removes a whole series of recordings from the schedule.
     * todo Exception
     *
    ...
    /**
     * Removes a recording from its associated series if it has one. Otherwise a call
     * to this method will be gently ignored.
     *
     * @param recording the recording to detach
     */
    ...
    /**
     * Saves a series. This method - in contrast to {@link #schedule(RecordingSeries)} - only makes
     * the series persistent, so it can be referred to it later on. Neither the series, nor the containing
     * recordings will be scheduled. Contained recording will <em>not</em> be saved.
     */
    void saveRecordingSeries(RecordingSeries series);

    /**
     * Hides a series from being displayed. The series will not be deleted, just flagged. Note that
     * only the series itself will be hidden, the contained recordings are not affected. 
     */
    ...
    /**
     * Creates a new event series with the given series id. See {@link EventSeries#setSeriesId(String)}
     * for further information on series ids.
     */
    EventSeries getOrCreateEventSeries(String seriesId);

    /**
     * Depending on the <code>content</code>'s type, different attachments will be returned,
     * e.g. if <code>content</code> is a {@link ch.ethz.replay.core.api.common.bundle.DublinCoreCatalog}
     * you'll get a {@link ch.ethz.replay.ui.scheduler.DublinCoreAttachment}.
     */
     ...
     *
     * @param preferredEmailAddress may be null
     */
