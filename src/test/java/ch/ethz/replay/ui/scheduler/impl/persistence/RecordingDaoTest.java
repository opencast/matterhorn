...
public class RecordingDaoTest {
    ...
    public void testDao() throws MetadataException {
        ...
        recordingDao.save(recording);
        assertEquals("Aurelien", ((DublinCoreAttachment) attachmentDao.get(recording.getDublinCoreAttachment().getId())).getContent().getFirst(PROPERTY_TITLE));
    ...
    public void testSeries() {
        ...
        recordingDao.save(recording);
        assertEquals(recording.getId(),
                recordingDao.get(recording.getId()).getSeries().getRecordings().iterator().next().getId());
